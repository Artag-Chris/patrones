...
class QueryBuilder {
  ...
  execute(): string {
    ...
    const orderByClouse = this.orderFields.length > 0
    ?`ORDER BY ${ this.orderFields.join(', ')}`
    :'';

    const limitClause = this.limitCount ? `LIMIT ${this.limitCount}`: '';

   return `Select ${fields} from ${this.table} ${whereClause} ${orderByClouse} ${limitClause}`
  }
}
...
function main() {
  const usersQuery = new QueryBuilder('users')
    .select('id', 'name', 'email')
    .where('age > 20')
    .where("country = 'COL'") // Esto debe de hacer una condición AND
    .orderBy('name', 'ASC')
    .limit(20)
    .execute();
